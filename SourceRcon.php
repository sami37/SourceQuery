<?php
	...
	class SourceRcon
	{
		...
		public function Open( )
		{
			if( !$this->RconSocket )
			{
				$this->RconSocket = @FSockOpen( $this->Address, $this->Port, $ErrNo, $ErrStr, $this->Timeout );
				...
				Stream_Set_Timeout( $this->RconSocket, $this->Timeout );
		...
		public function Command( $Command )
		{
			...
			else if( $Type === SourceQuery::SERVERDATA_ANOTHER_RESPONSE_VALUE )
			{
				// var_dump($Buffer->Get());
			}
			else if( $Type !== SourceQuery::SERVERDATA_RESPONSE_VALUE )
			{
				throw new InvalidPacketException( 'Invalid rcon response. Type: '.$Type, InvalidPacketException::PACKET_HEADER_MISMATCH );
			}
